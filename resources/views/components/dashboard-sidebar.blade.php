...
<aside x-cloak :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
    class="fixed lg:static lg:translate-x-0 z-40 transition-transform w-72 bg-ink-900 text-white min-h-screen p-6">
    ...
    <nav class="space-y-1 text-sm">
        @foreach($links as [$label, $href, $icon])
            <a href="{{ url($href) }}"
                class="flex items-center gap-3 px-4 py-3 rounded-xl {{ request()->is(ltrim($href, '/')) ? 'bg-gold-500 text-white' : 'text-white/70 hover:bg-white/5 hover:text-white' }} transition">
    ...
    @php
        $sidebarRate = \App\Models\GoldRate::latest()->first();
    @endphp
    <div class="mt-10 p-4 rounded-xl bg-gradient-to-br from-gold-500 to-gold-600 text-xs">
        ...
        @if($sidebarRate)
        <p class="text-lg font-serif font-bold">₹{{ number_format($sidebarRate->rate_22k, 2) }} <span class="text-xs font-normal">/gm (22K)</span></p>
        @else
        <p class="text-sm opacity-80">Rate not published yet</p>
        @endif
    </div>
</aside>